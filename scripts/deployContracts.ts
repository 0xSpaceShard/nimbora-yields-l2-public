...
async function deployPoolingManagerContract(): Promise<Contract> {
  ...
  const compiledContract = await json.parse(
    fs
      .readFileSync(
        `./target/dev/nimbora_yields_PoolingManager.contract_class.json`
      )
      .toString('ascii')
  );
  ...
  let { transaction_hash, contract_address } = await owner.deploy({
    classHash: config[network].POOLING_MANAGER_CLASS_HASH as string,
    constructorCalldata: {
      owner: owner.address,
    },
  });
  ...
  console.log(
    '✅ Test PoolingManager contract connected at =',
    poolingManagerContract.address
  );
...
async function deployFactoryContract(): Promise<Contract> {
  ...
  const compiledContract = await json.parse(
    fs
      .readFileSync(`./target/dev/nimbora_yields_Factory.contract_class.json`)
      .toString('ascii')
  );
  let configs = readConfigs();
  ...
  let { transaction_hash, contract_address } = await owner.deploy({
    classHash: configs[network].FACTORY_CLASS_HASH as string,
    constructorCalldata: {
      pooling_manager: l2PoolingManager as string,
      token_class_hash: configs[network].TOKEN_CLASS_HASH as string,
      token_manager_class_hash: configs[network]
        .TOKEN_MANAGER_CLASS_HASH as string,
    },
  });
  [contractAddress] = contract_address;
  await provider.waitForTransaction(transaction_hash);
  ...
  console.log(
    '✅ Test Factory contract connected at =',
